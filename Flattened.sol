...
contract Gen {
    ...
    uint16[270] ps = [
        1059, 1883, 2942, 3648, 5413, 6472, 7648, 9177, 9765, 10000,
        385, 961, 1443, 1828, 2790, 4905, 6155, 7597, 9616, 10000,
        198, 792, 1584, 2574, 3267, 3366, 5643, 7029, 9900, 10000,
        734, 1101, 1651, 2293, 3027, 4403, 5320, 6329, 9173, 10000,
        386, 1351, 3474, 5212, 7143, 9846, 9943, 9962, 9981, 10000,
        14, 157, 869, 1013, 1726, 2582, 4294, 5578, 7718, 10000,
        2404, 2885, 3847, 5129, 6571, 8334, 9776, 9936, 9984, 10000,
        1246, 1780, 2670, 4093, 5339, 6762, 7296, 9965, 9983, 10000,
        266, 319, 385, 1715, 4774, 7433, 9961, 9974, 9987, 10000,
        392, 588, 1078, 1568, 2254, 3038, 3920, 5195, 6764, 10000,
        712, 1941, 5013, 7613, 9946, 9964, 9973, 9982, 9991, 10000,
        755, 1227, 1603, 1887, 2076, 2170, 3113, 9245, 9811, 10000,
        695, 747, 834, 851, 3023, 5195, 6846, 9974, 9991, 10000,
        103, 514, 822, 1438, 2567, 3902, 7290, 9959, 9980, 10000,
        587, 734, 749, 1336, 2070, 2804, 4126, 6182, 8238, 10000,
        88, 1150, 2566, 2743, 3982, 4690, 4867, 5397, 7168, 10000,
        325, 760, 1303, 1954, 3366, 4669, 6623, 8252, 9989, 10000,
        9911, 9921, 9931, 9941, 9951, 9961, 9971, 9981, 9991, 10000,
        212, 424, 636, 657, 1188, 1506, 2567, 3630, 4692, 10000,
        200, 800, 2599, 3399, 4399, 5598, 7796, 9996, 9998, 10000,
        475, 1424, 1661, 2848, 4272, 5933, 8544, 9968, 9992, 10000,
        526, 631, 1157, 1472, 2631, 3684, 5368, 6947, 8210, 10000,
        202, 1412, 3025, 5444, 9879, 9919, 9939, 9959, 9979, 10000,
        23, 480, 2657, 2886, 4719, 7354, 9645, 9874, 9885, 10000,
        433, 866, 1732, 5195, 8658, 9524, 9697, 9870, 9957, 10000,
        631, 946, 1577, 2208, 3785, 6309, 9464, 9779, 9874, 10000,
        204, 511, 714, 1737, 3782, 9917, 9968, 9978, 9988, 10000];
    ...
    function determineWordLength(uint rand) internal pure returns(uint) {
        ...
        uint16[16] memory cumulativeDistribution = [1,98,760,1928,3174,4693,6290,7653,8743,9327,9677,9871,9937,9975,9990,10000];
    ...
    function generateWord(uint256 _tokenId, uint _totalSeed) internal view returns(string memory) { // change visibility
        ...
        //require(_tokenId <= nextTokenId, "NFT with this tokenId is yet to be minted.");
        require(_tokenId >= 1 && _tokenId <= 10000, "Invalid tokenId.");

        string memory word;
        string memory char;
        
        uint lengthRand = (uint(keccak256(abi.encodePacked(_tokenId, _totalSeed)))% 10000); // gets random number between 1 and 10,000
        ...
        for (uint n=1; n <= determineWordLength(lengthRand); n++) {
            ...
            // generates letter
            uint i = 0;
            while (i < 10) { // indexStart of ps[] to indexEnd
                if (rand <= ps[(charsMap[char]*10)+i]) {
                    break;
                }
                i++;
            }
...
contract Lootlang is ERC721, Ownable, Gen {
    ...
    uint public contractBalance; // smart contract ether balance [wei]
    ...
    constructor() Ownable() ERC721('Lootlang', 'LANG') {
        ...
        freezeBlock = 13406398; // SET FREEZE BLOCK 
